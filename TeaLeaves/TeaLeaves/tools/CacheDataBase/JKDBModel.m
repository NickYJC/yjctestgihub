...
+ (NSArray *)findByCriteria:(NSString *)criteria
{
    ...
    [jkDB.dbQueue inDatabase:^(FMDatabase *db) {
        NSString *tableName = NSStringFromClass(self.class);
        //URLRequest WHERE Url = ?;
        // NSString *sql = [NSString stringWithFormat:@"SELECT * FROM %@ %@",tableName,criteria];
        //NSString * sql = @"SELECT ItemData FROM URLRequest WHERE Url = ?;";//klineID
        
        NSString *sql =[NSString stringWithFormat:@"SELECT %@ FROM URLRequest WHERE Url = ?;",tableName];
        // NSString *sql = [NSString stringWithFormat:@"SELECT * FROM URLRequest WHERE Url = ?;",tableName,url];
        // FMResultSet *resultSet = [db executeQuery:sql];
        
        FMResultSet * resultSet = [db executeQuery:sql, criteria];
        while ([resultSet next]) {
            JKDBModel *model = [[self.class alloc] init];
            for (int i=0; i< model.columeNames.count; i++) {
                NSString *columeName = [model.columeNames objectAtIndex:i];
