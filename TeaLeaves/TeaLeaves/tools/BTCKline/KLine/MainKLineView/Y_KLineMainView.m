...
- (CGFloat)getExactXPositionWithOriginXPosition:(CGFloat)originXPosition{
    ...
    for (NSInteger index = startIndex > 0 ? startIndex - 1 : 0; index < arrCount; ++index) {
        ...
        {
            if(self.delegate && [self.delegate respondsToSelector:@selector(kLineMainViewLongPressKLinePositionModel:kLineModel:withPeciprocalModel:kLineModelIndex:)])
            {
                
                
                if (index>0) {
                    [self.delegate kLineMainViewLongPressKLinePositionModel:self.needDrawKLinePositionModels[index] kLineModel:self.needDrawKLineModels[index]withPeciprocalModel:self.needDrawKLineModels[index-1] kLineModelIndex:index];
                }
                else
                {
                    [self.delegate kLineMainViewLongPressKLinePositionModel:self.needDrawKLinePositionModels[index] kLineModel:self.needDrawKLineModels[index]withPeciprocalModel:nil kLineModelIndex:index];
                }
            }
...
- (CGFloat)getExactXXXPositionWithIndex:(NSInteger)index
{
   
    if (self.needDrawKLinePositionModels.count>index) {
         Y_KLinePositionModel *kLinePositionModel = self.needDrawKLinePositionModels[index];
...
- (CGFloat)getExactYYYPositionWithIndex:(NSInteger)index
{
...
#pragma mark-时间yjc2016年10月25日19:14:01
- (CGFloat)getExactXPositionWithOriginYPosition:(CGFloat)originYPosition
{
    CGFloat yPositoinInMainView = originYPosition;
    NSInteger startIndex = (NSInteger)((yPositoinInMainView - self.startXPosition) / ([Y_StockChartGlobalVariable kLineGap] + [Y_StockChartGlobalVariable kLineWidth]));
     // DebugLog(@"yjc11");
    NSInteger arrCount = self.needDrawKLinePositionModels.count;
...
//    __block NSInteger  minkLineModelNum=0;
//    __block NSInteger  maxkLineModelNum=0;
    
    
    
    Y_KLineModel *firstModel = kLineModels.firstObject;
     CGFloat minAssert = firstModel.Low.floatValue;
     CGFloat maxAssert = firstModel.High.floatValue;
    //    __block CGFloat minMA7 = CGFLOAT_MAX;
    //    __block CGFloat maxMA7 = CGFLOAT_MIN;
    //    __block CGFloat minMA30 = CGFLOAT_MAX;
    //    __block CGFloat maxMA30 = CGFLOAT_MIN;
     NSInteger  minkLineModelNum=0;
     NSInteger  maxkLineModelNum=0;
//    [kLineModels enumerateObjectsUsingBlock:^(Y_KLineModel * _Nonnull kLineModel, NSUInteger idx, BOOL * _Nonnull stop) {
    for (NSInteger idx = 0 ; idx < kLineModels.count; ++idx){
        ...
        if(kLineModel.High.floatValue > maxAssert)
        {
            ...
            [AppDefault shareDefault].reciprocalMaxModel = kLineModel;
            maxkLineModelNum =idx;
        }
        if(kLineModel.Low.floatValue < minAssert)
        {
            minAssert = kLineModel.Low.floatValue;
            [AppDefault shareDefault].reciprocalMinModel = kLineModel;
             minkLineModelNum =idx;
        }
...
//    }];
   ...
   // DebugLog(@"minLow=%@ %f",[AppDefault shareDefault].reciprocalMinModel.Date,[[AppDefault shareDefault].reciprocalMinModel.Low floatValue]);
   //  DebugLog(@"maxHigh====%@ %f",[AppDefault shareDefault].reciprocalMinModel.Date,[[AppDefault shareDefault].reciprocalMaxModel.High floatValue]);
    maxAssert *= 1.0001;
    minAssert *= 0.9991;
    ...
    for (NSInteger idx = 0 ; idx < kLineModelsCount; ++idx)
    {
        ...
        if (idx == minkLineModelNum) {
             [AppDefault shareDefault].reciprocalMinPoint =lowPoint;
            // DebugLog(@"min=3333333333=%@ %@",[AppDefault shareDefault].reciprocalMinModel.Date,kLineModel.Date);
        }
        if (idx == maxkLineModelNum) {
              [AppDefault shareDefault].reciprocalMaxPoint =highPoint;
             //DebugLog(@"max=3333333333=%@ %@",[AppDefault shareDefault].reciprocalMaxModel.Date,kLineModel.Date);
        }
       
//        if ([ [AppDefault shareDefault].reciprocalMinModel.Date isEqualToString:kLineModel.Date]) {
//
//
//        }
//        if ([ [AppDefault shareDefault].reciprocalMaxModel.Date isEqualToString:kLineModel.Date]) {
//            [AppDefault shareDefault].reciprocalMinPoint =highPoint;
//
//        }
    }
