...
-(void)maProfileWithModel:(Y_KLineModel *)model
{
    if(self.targetLineStatus == Y_StockChartTargetLineStatusMACD)
    {
        ...
        _accessoryDescLabel.text = @" RSI(N1:6,N2:12,N3:24)";
        _DIFLabel.text = [NSString stringWithFormat:@"  N1：%.4f ",model.RSI_6.floatValue];
        _DEALabel.text = [NSString stringWithFormat:@"  N2：%.4f",model.RSI_12.floatValue];
        _MACDLabel.text = [NSString stringWithFormat:@"  N3：%.4f",model.RSI_24.floatValue];
    }
    else  if(self.targetLineStatus == Y_StockChartTargetLineStatusKDJ&&[AppDefault shareDefault].RSIOrWRNum==WRNUM){
        _accessoryDescLabel.text = @" WR(N1:10,N2:6)";
        _DIFLabel.text = [NSString stringWithFormat:@"  N1：%.4f ",model.WR_10.floatValue];
        _DEALabel.text = [NSString stringWithFormat:@"  N2：%.4f",model.WR_6.floatValue];
        _MACDLabel.text =@"";
    }
